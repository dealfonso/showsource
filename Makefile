...
ifneq ("","$(wildcard js/*.js)")
	uglifyjs -e window,document:window,document js/*.js $(UGLIFY_FLAGS) -b | cat notice - > $(FILE_NAME).js
	uglifyjs -e window,document:window,document js/*.js $(UGLIFY_FLAGS_MIN) | cat notice.min - > $(FILE_NAME).min.js
endif
